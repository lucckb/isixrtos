...
BaseType_t xError = pdFALSE;
	...
	for( ;; )
	{
		...
		***********************************************************************
		...
		if( ( uxReturned & ebALL_SYNC_BITS ) != ebALL_SYNC_BITS )
		{
			dbprintf("Error %08x xEvebtGroup %p", uxReturned , xEventGroup );
			xError = pdTRUE;
		}
...
EventBits_t uxBit;
	...
	for( uxBit = 0x01; uxBit < 0x100; uxBit <<= 1 )
	{
		...
		{
			xError = pdTRUE;
			dbprintf("Error eTaskSt %i", eTaskGetState(xSyncTask1) );
		}
		...
		if( ( uxBit & ebSELECTIVE_BITS_1 ) == 0 )
		{
			...
			{
				xError = pdTRUE;
				dbprintf("Error %i", eTaskGetState(xSyncTask1) );
			}
		}
		else
		{
