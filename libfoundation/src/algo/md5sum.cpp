...
*/
#include <foundation/algo/md5sum.hpp>
#include <cstring>
...
MD5::MD5(const unsigned char* str, size_t len )
{
  init();
  update( str, len );
  finalize();
...
MD5::MD5( const char *str ) 
	: MD5( reinterpret_cast<const unsigned char*>(str), std::strlen(str) )
{
...
const char* MD5::hexdigest( char* buf, size_t len )
{
	if (!finalized)
		return nullptr;
	for (int i=0; i<16 && len>2; i++,len-=2 )
