 ...
 *
 *        Version:  1.0
 *        Created:  06/13/2013 12:11:29 PM
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  YOUR NAME (), 
...
int         gen_fnt_file(font_t *pf, char *path);
...
gen_c_source(font_t *pf, char *path)
{
        ...
        char hdr1[] = {
        		...
        		"namespace gfx { \n"
        		"namespace fonts {\n\n"
                "static const unsigned short %s_bits[] = {\n"
        };
        ...
        if (pf->width) {
                fprintf(ofp,    "/* Character width data. */\n"
                        "static const unsigned char %s_width[] = {\n",
                        pf->name);
...
int
gen_fnt_file(font_t *pf, char *path)
{
        FILE *ofp;
        int i;
        ...
        WRITELONG(ofp, pf->bits_size);            /* # words of bits[]*/
